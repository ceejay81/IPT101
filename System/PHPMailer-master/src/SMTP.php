...
class SMTP
{
     ...
     * @see    hello()
     *
     * @param string $student_id The user name
    ...
    public function authenticate(
        ...
        switch ($authtype) {
            case 'PLAIN':
                ...
                //Send encoded student_id and password
                if (
                    //Format from https://tools.ietf.org/html/rfc4616#section-2
                    //We skip the first field (it's forgery), so the string starts with a null byte
                    !$this->sendCommand(
                        'User & Password',
                        base64_encode("\0" . $student_id . "\0" . $password),
                        235
                    )
                ) {
                    return false;
            ...
            case 'LOGIN':
                ...
                if (!$this->sendCommand('student_id', base64_encode($student_id), 334)) {
                    return false;
                }
            ...
            case 'CRAM-MD5':
                ...
                //Build the response
                $response = $student_id . ' ' . $this->hmac($challenge, $password);

                //send encoded credentials
                return $this->sendCommand('student_id', base64_encode($response), 235);
            case 'XOAUTH2':
    ...
    public function client_send($data, $command = '')
    {
        ...
        if (
            ...
            in_array($command, ['User & Password', 'student_id', 'Password'], true)
        ) {
