...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    ...
    } elseif (!preg_match('/^[a-zA-Z]+$/', $student_id)) {
        $errors['student_id'] = "student_id should only contain letters!";
        ...
        if (mysqli_num_rows($email_check_result) > 0) {
            ...
            if ($result) {
                ...
                $sql = "INSERT INTO ipt102_db (student_id, password, first_name, last_name, middle_name, email, status, active) 
                        VALUES ('$student_id', '$hashed_password', '$firstName', '$lastName', '$middleName', '$email', '$status', '$active')";
                $result = mysqli_query($conn, $sql);
                ...
                if ($result) {
                    ...
                    $mail->SMTPAuth = true;
                    $mail->student_id = 'jakeastimoso@gmail.com';
                    $mail->Password = 'jqnozkpzgzgiuqal';
                    $mail->SMTPSecure = 'tls';
                    $mail->Port = 587;
                    
                    $mail->setFrom('jakeastimoso@gmail.com');
                    $mail->addAddress($email);
                    $mail->isHTML(true);
                    $mail->Subject = 'Email Verification';
                    $mail->Body = 'Please click the "verify" link to verify your email: <a href="http://localhost/loginregister/verify.php?email=' . urlencode($email) . '&code=' . $verification_code . '">Verify</a>';
     
                    try {
                        $mail->send();
                        $success_message = "Verification email sent. Please check your email to verify your account.";
                    } catch (Exception $e) {
                        $errors['email'] = "Failed to send verification email. Error: " . $mail->ErrorInfo;
                    }
                } else {
            ...
            <div class="col-md-4 offset-md-4 form register-form">
                <form action="register.php" method="POST" autocomplete="off"> 
                    ...
                    <div class="form-group">
                        <input class="form-control" type="text" name="student_id" placeholder="Student ID" required value="<?php echo isset($_POST['student_id']) ? $_POST['student_id'] : ''; ?>">
                    </div>
                    <div class="form-group">
